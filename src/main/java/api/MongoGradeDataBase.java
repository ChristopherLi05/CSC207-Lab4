    ...
    public Team getMyTeam() {
        ...
        try {
            ...
            if (responseBody.getInt(STATUS_CODE) == SUCCESS_CODE) {
                ...
                JSONArray arr = teamInfo.getJSONArray("members");
                String[] members = new String[arr.length()];
                for (int i = 0; i < arr.length(); i++) {
                    members[i] = arr.getString(i);
                }

                return Team.builder().name(teamInfo.getString("name")).members(members).build();
            }
            else {
                throw new RuntimeException(responseBody.getString(MESSAGE));
            }
